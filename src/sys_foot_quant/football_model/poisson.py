"""Modele Poisson simple : attaque/defense par equipe + HFA a shrinkage bayesien.

Formulation retenue (methode des ratios a la moyenne de ligue, pas de
maximum de vraisemblance joint) :

    lambda(domicile=h, exterieur=a) = league_base * attack[h] * defense[a] * hfa_team[h]
    mu(domicile=h, exterieur=a)     = league_base * attack[a] * defense[h]

Ou :
- ``league_base`` est le nombre moyen de buts par equipe et par match,
  toutes rencontres confondues (domicile et exterieur traites de facon
  symetrique) ;
- ``attack[t]`` / ``defense[t]`` sont des ratios centres sur 1.0 (equipe
  moyenne), calcules a partir de TOUS les matchs de l'equipe (domicile +
  exterieur), donc independants du lieu par construction ;
- ``hfa_team[h]`` capture entierement l'effet du terrain : c'est un ratio
  brut par equipe (A2 du Research Framework), shrink vers ``hfa_global``
  (moyenne de ligue) proportionnellement au nombre de matchs a domicile
  observes pour cette equipe (shrinkage bayesien empirique, formule
  poids/(poids+k)).

C'est un choix de modelisation delibere pour rester simple (pas
d'optimiseur numerique, pas de risque de non-convergence) : voir
docs/research_framework.md pour l'analyse des alternatives (MLE jointe,
Dixon-Coles) et le choix argumente de rester a ce niveau de simplicite
pour l'etape 2. Une limite connue et documentee : cette formulation ne
garantit pas de reconstruire exactement les moyennes marginales brutes
(avg_home_goals / avg_away_goals) - seule une estimation par MLE jointe le
garantirait. La validation se fait empiriquement (Brier/log loss hors
echantillon), pas par une identite algebrique.
"""

from __future__ import annotations

import numpy as np
import pandas as pd

from sys_foot_quant.football_model.scoring import outcome_probabilities, score_matrix
from sys_foot_quant.football_model.weighting import flat_weights

_EPS = 1e-9


class PoissonModel:
    def __init__(self, hfa_shrinkage_k: float = 10.0, use_team_hfa: bool = True) -> None:
        if hfa_shrinkage_k <= 0:
            raise ValueError("hfa_shrinkage_k doit etre strictement positif.")
        self.hfa_shrinkage_k = hfa_shrinkage_k
        self.use_team_hfa = use_team_hfa

        self.attack_: dict[int, float] | None = None
        self.defense_: dict[int, float] | None = None
        self.league_base_: float | None = None
        self.hfa_global_: float | None = None
        self.hfa_team_: dict[int, float] | None = None
        self.raw_hfa_: dict[int, float] | None = None
        self.n_home_: dict[int, float] | None = None

    def fit(self, matches_df: pd.DataFrame, weights: np.ndarray | None = None) -> "PoissonModel":
        if matches_df.empty:
            raise ValueError("Impossible d'entrainer sur un ensemble de matchs vide.")

        n = len(matches_df)
        w = flat_weights(n) if weights is None else np.asarray(weights, dtype=float)
        if w.shape[0] != n:
            raise ValueError(f"weights doit avoir {n} elements, recu {w.shape[0]}.")
        total_weight = float(w.sum())
        if total_weight <= 0:
            raise ValueError("La somme des ponderations doit etre strictement positive.")

        home_ids = matches_df["home_team_id"].to_numpy()
        away_ids = matches_df["away_team_id"].to_numpy()
        home_goals = matches_df["home_goals"].to_numpy(dtype=float)
        away_goals = matches_df["away_goals"].to_numpy(dtype=float)

        league_base = max(float((w * (home_goals + away_goals)).sum()) / (2.0 * total_weight), _EPS)
        mean_home = float((w * home_goals).sum()) / total_weight
        mean_away = float((w * away_goals).sum()) / total_weight
        hfa_global = max(mean_home / mean_away, _EPS) if mean_away > 0 else 1.0

        teams = sorted(set(home_ids.tolist()) | set(away_ids.tolist()))
        scored_sum = {t: 0.0 for t in teams}
        conceded_sum = {t: 0.0 for t in teams}
        weight_total = {t: 0.0 for t in teams}
        for i in range(n):
            h, a, wi = home_ids[i], away_ids[i], w[i]
            scored_sum[h] += wi * home_goals[i]
            scored_sum[a] += wi * away_goals[i]
            conceded_sum[h] += wi * away_goals[i]
            conceded_sum[a] += wi * home_goals[i]
            weight_total[h] += wi
            weight_total[a] += wi

        attack: dict[int, float] = {}
        defense: dict[int, float] = {}
        for t in teams:
            if weight_total[t] <= 0:
                attack[t] = 1.0
                defense[t] = 1.0
            else:
                scored_rate = scored_sum[t] / weight_total[t]
                conceded_rate = conceded_sum[t] / weight_total[t]
                attack[t] = max(scored_rate / league_base, _EPS)
                defense[t] = max(conceded_rate / league_base, _EPS)

        self.attack_ = attack
        self.defense_ = defense
        self.league_base_ = league_base
        self.hfa_global_ = hfa_global

        home_weight = {t: 0.0 for t in teams}
        home_weighted_ratio_sum = {t: 0.0 for t in teams}
        for i in range(n):
            h, a, wi = home_ids[i], away_ids[i], w[i]
            expected_neutral = league_base * attack[h] * defense[a]
            ratio = home_goals[i] / expected_neutral
            home_weight[h] += wi
            home_weighted_ratio_sum[h] += wi * ratio

        raw_hfa: dict[int, float] = {}
        n_home: dict[int, float] = {}
        hfa_team: dict[int, float] = {}
        for t in teams:
            n_home[t] = home_weight[t]
            raw_hfa[t] = (
                home_weighted_ratio_sum[t] / home_weight[t] if home_weight[t] > 0 else hfa_global
            )
            if self.use_team_hfa:
                k = self.hfa_shrinkage_k
                hfa_team[t] = (n_home[t] * raw_hfa[t] + k * hfa_global) / (n_home[t] + k)
            else:
                hfa_team[t] = hfa_global

        self.raw_hfa_ = raw_hfa
        self.n_home_ = n_home
        self.hfa_team_ = hfa_team
        return self

    def predict_lambda_mu(self, home_team_id: int, away_team_id: int) -> tuple[float, float]:
        if self.attack_ is None:
            raise RuntimeError("Le modele doit etre entraine (fit) avant predict_lambda_mu().")
        assert self.defense_ is not None
        assert self.hfa_team_ is not None
        assert self.league_base_ is not None
        assert self.hfa_global_ is not None

        attack_h = self.attack_.get(home_team_id, 1.0)
        defense_h = self.defense_.get(home_team_id, 1.0)
        attack_a = self.attack_.get(away_team_id, 1.0)
        defense_a = self.defense_.get(away_team_id, 1.0)
        hfa_h = self.hfa_team_.get(home_team_id, self.hfa_global_)

        lam = self.league_base_ * attack_h * defense_a * hfa_h
        mu = self.league_base_ * attack_a * defense_h
        return lam, mu

    def predict_outcome_probabilities(
        self, home_team_id: int, away_team_id: int, max_goals: int = 20
    ) -> tuple[float, float, float]:
        lam, mu = self.predict_lambda_mu(home_team_id, away_team_id)
        matrix = score_matrix(lam, mu, max_goals=max_goals)
        home_win, draw, away_win = outcome_probabilities(matrix)
        # La troncature de score_matrix laisse une masse residuelle
        # negligeable mais non nulle (queue de la distribution au-dela de
        # max_goals) : on renormalise pour garantir une distribution de
        # probabilite propre (somme exactement 1), condition exigee par le
        # Calibration Engine.
        total = home_win + draw + away_win
        return (home_win / total, draw / total, away_win / total)

    def predict(self, home_team_id: int, away_team_id: int) -> tuple[float, float, float]:
        """Alias de ``predict_outcome_probabilities`` (max_goals par defaut),
        pour une interface uniforme avec NaiveModel/EloModel dans le
        walk-forward (``FittedPredictor.predict``)."""
        return self.predict_outcome_probabilities(home_team_id, away_team_id)
