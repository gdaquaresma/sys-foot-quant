...
def test_load_basic_rows(tmp_path: Path) -> None:
    rows = [
        "E0,16/08/2024,20:00,Man United,Fulham,1,0,H,1.6,4.2,5.25,1.66,4.15,5.33,1.68,4.5,5.6,1.62,4.36,5.15,1.85,1.95,1.88,1.92",
        "E0,17/08/2024,12:30,Ipswich,Liverpool,0,2,A,8.5,5.5,1.33,8,5.5,1.35,9,6.1,1.37,8.28,5.76,1.34,1.5,2.5,1.52,2.45",
    ]
...
def test_missing_b365_values_are_flagged_not_dropped(tmp_path: Path) -> None:
    rows = ["E0,16/08/2024,20:00,Man United,Fulham,1,0,H,,,,"
            "1.66,4.15,5.33,1.68,4.5,5.6,1.62,4.36,5.15,,,1.88,1.92"]
    path = _write_csv(tmp_path / "E0.csv", rows)
    records = load_football_data_csv(path, league="premier_league", season="2024_25")
...
def test_closing_odds_columns_are_never_read_even_when_present(tmp_path: Path) -> None:
    ...
    doit produire QUE B365H/D/A (et B365>2.5/B365<2.5 pre-match) - jamais
    B365CH/CD/CA ni B365C>2.5/B365C<2.5."""
    rows = [
        "E0,16/08/2024,20:00,Man United,Fulham,1,0,H,1.6,4.2,5.25,"
        "99.9,99.9,99.9,1.68,4.5,5.6,1.62,4.36,5.15,1.85,1.95,99.9,99.9"
    ]
    path = _write_csv(tmp_path / "E0.csv", rows)
    records = load_football_data_csv(path, league="premier_league", season="2024_25")
